...
// 1. Add state variables to AdminPortal
const stateTarget = `const [waTogglingAi, setWaTogglingAi] = useState(false);`;
const stateReplacement = `const [waTogglingAi, setWaTogglingAi] = useState(false);
...
const funcReplacement = funcTarget + `
      ...
      const handleToggleAiFollowUp = async () => {
        ...
        try {
          const nextState = waStatus.aiFollowUpEnabled === false ? true : false;
          ...
          alert('Failed to toggle AI Follow-up state');
        } finally {
...
// 3. Add UI Cards
const buttonTarget = `                                  {waTogglingAi 
                                    ? 'Updating...' 
...
console.log('SUCCESS: Cleanly patched build_luxury_theme.js!');
