...
// 4. Update submit button with loading state
...
if (content.includes(btnTarget)) {
  ...
  console.log('SUCCESS: Updated submit button loading state');
}

// 5. Update text input for Image URL so it displays clean preview text when base64
const textTarget = `value={prodForm.image || ''}`;
const textReplacement = `value={prodForm.image && prodForm.image.startsWith('data:image') ? '[Uploaded Image Selected]' : (prodForm.image || '')}`;

if (content.includes(textTarget)) {
  content = content.replace(textTarget, textReplacement);
  console.log('SUCCESS: Cleaned up Image URL text field display');
}
