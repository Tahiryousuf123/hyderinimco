...
// 1. Remove old standalone sticky nav div
...
// 2. Insert new integrated Category Navigation Bar inside <header>
const headerTarget = `          </header>`;

const categoryNavCode = `            {/* Integrated Category Navigation Bar */}
...
if (content.includes(headerTarget)) {
  content = content.replace(headerTarget, categoryNavCode);
  console.log('SUCCESS: Inserted category bar inside header');
} else {
  console.error('ERROR: headerTarget not found');
}
