...
// 1. Remove old standalone category nav
const oldNavBlock = `            {/* Category Navigation Pills */}
...
if (content.includes(oldNavBlock)) {
  content = content.replace(oldNavBlock, '');
  console.log('SUCCESS: Removed old category nav block');
} else {
  console.error('ERROR: oldNavBlock not found');
}

// 2. Add Category Navigation Bar inside <header>
const headerClosing = `          </header>`;
const newCategoryBar = `            {/* Integrated Category Navigation Bar */}
...
if (content.includes(headerClosing)) {
  content = content.replace(headerClosing, newCategoryBar);
  console.log('SUCCESS: Added new category bar inside header');
} else {
  console.error('ERROR: headerClosing not found');
}
